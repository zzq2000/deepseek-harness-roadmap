...
flowchart LR
    subgraph R["左右对照阅读"]
        direction TB
        A[教程正文] <-->|点击源码引用| B[右侧 IDE<br/>定位并高亮到行]
    end

    R --> C[选择题<br/>本地即时判分]
    C --> D[开放问答]
    D -->|答案 + 该题源码上下文| E[DeepSeek 判卷]
    E --> F[得分 · 逐点批改<br/>漏掉的关键点]
    F -->|不足 70| R
    F -->|达标| G([章节完成])

    classDef judge fill:#4d6bfe,stroke:#3a53c4,color:#fff
    classDef done fill:#17864b,stroke:#0f6337,color:#fff
    class E judge
    class G done
    style R fill:none,stroke:#8b93a1,stroke-dasharray:5 5
```
